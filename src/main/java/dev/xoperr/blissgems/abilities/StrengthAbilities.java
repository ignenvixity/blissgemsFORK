...
public class StrengthAbilities {
    ...
    public void onRightClick(Player player, int tier) {
        if (tier == 2 && player.isSneaking()) {
            ...
            // Tier 2 without sneak: try Frailer first (single target), fallback to Bloodthorns.
    ...
    public void bloodthorns(Player player) {
        ...
        for (int i = 0; i < 16; i++) {
            ...
            player.getWorld().spawnParticle(Particle.DUST, player.getLocation().add(x, 0.5, z), 3, 0.1, 0.1, 0.1, 0.0, redDust, true);
        }
